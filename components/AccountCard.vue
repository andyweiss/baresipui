<template>
  <div class="bg-white rounded-lg shadow-lg p-6 border-l-4" :class="borderColor">
    <div class="flex items-start justify-between mb-4">
      <div class="flex-1">
        <h3 class="text-lg font-semibold text-gray-800 mb-1">{{ accountName }}</h3>
        <p class="text-sm text-gray-500 font-mono break-all">{{ account.uri }}</p>
      </div>
      <div class="ml-4">
        <span
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      <div>
        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Call Status</p>
        <p class="text-sm font-medium" :class="callStatusColor">{{ account.callStatus || 'Idle' }}</p>
      </div>
      <div>
        <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Auto-Connect</p>
...
const callStatusColor = computed(() => {
  ...
  if (status === 'Ringing') return 'text-yellow-600';
