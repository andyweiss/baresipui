			  ...
			  const struct sipevent_substate *substate, void *arg)
{
	...
	if (substate) {
		...
		/* 404/481 are considered transient in this setup (peer offline / dialog state).
		   Retry frequently and don't escalate fail counter. */
		if (msg->scode == 404 || msg->scode == 481) {
			pres->failc = 0;
			wait = 30;
		}
		else {
